...
function endGame() {
    ...
    // --- DOM要素の取得をこの関数内に移動 (推奨) ---
    const finalScoreDisplay = document.getElementById('finalScore');
    const endTimeDisplay = document.getElementById('endTime');
    const prizeMessageArea = document.getElementById('prizeMessage');
    ...
    // 3. 特典メッセージをスコアに応じて設定
    let messageHTML = '';
    const PRIZE_THRESHOLD = 30; // 30点以上

    if (score >= PRIZE_THRESHOLD) {
        messageHTML = `
            <p>🎉おめでとうございます！🎉</p>
            <p><span class="prize-highlight">３０点以上</span>を達成しました！</p>
            <p>この画面をスタッフにご提示で、<span class="prize-highlight">１００円OFFクーポン</span>を差し上げます！</p>
        `;
    } else {
